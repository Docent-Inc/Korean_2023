...
def train(
    ...
    for i in range(1, k+1):
        ...
        # Load all the folds
        datasets = [load_dataset("json", data_files=os.path.join(data_path, f"td_fold_{j}.jsonl"))["train"] for j in range(1, k+1)]
        val_data = datasets[i-1]
        val_set_size = len(val_data)

        train_data = datasets[0:i-1] + datasets[i:]
